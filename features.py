...
def users_fullname(meeting):
    if pd.isnull(meeting["username"]) or pd.isnull(meeting["meetingtitle"]):
        return 0
    if str(meeting["username"]).lower() in str(meeting["meetingtitle"]).lower():
        return 1
    else:
        return 0
...
def word_in_title(word, meeting):
    if pd.isnull(meeting["meetingtitle"]):
        return 0
    return word.lower() in str(meeting["meetingtitle"]).lower()


# returns 1 if any word in word_list is included in the meeting title
def word_list_check(meeting, *word_list):
...
def user_company_in_title(meeting):
    if not pd.isnull(meeting["companyname"]) and  pd.isnull(meeting["meetingtitle"]):
        ...
        if str(meeting['companyname']).lower() in str(meeting["meetingtitle"]).lower():
            return 1
        else:
            return 0
    else:
